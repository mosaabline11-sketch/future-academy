...
const ADMIN_PASSWORD = "faweb2026"; // ← غيّر كلمة المرور هنا
...
async function sb(table, method, body, filter, extra) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn("[SB] " + method + " " + table + filter + " ->", res.status);
      return null;
    }
    const text = await res.text();
    return text ? JSON.parse(text) : true;
  } catch (e) {
    console.warn("[SB] error:", e.message);
    return null;
  }
}
  ...
  for (var li = 0; li <= maxLine; li++) {
    ...
    slots.push(
      lineRows.map(function (r) {
        return { pos: r.pos, pid: r.player_id || null };
      }),
    );
  }
  return slots;
...
const POS_CFG = {
  "حارس مرمى": {
    ...
    cardBg: "linear-gradient(160deg,#1a0a00,#2d1400,#1a0a00)",
  },
  مدافع: {
    badge: "DEF",
    color: "#3b82f6",
    glow: "rgba(59,130,246,.5)",
    cardBg: "linear-gradient(160deg,#00081a,#001030,#00081a)",
  },
  "وسط ملعب": {
    badge: "MID",
    color: "#10b981",
    glow: "rgba(16,185,129,.5)",
    cardBg: "linear-gradient(160deg,#001a0d,#002a15,#001a0d)",
  },
  مهاجم: {
    badge: "FWD",
    color: "#ef4444",
    glow: "rgba(239,68,68,.5)",
    cardBg: "linear-gradient(160deg,#1a0000,#2d0000,#1a0000)",
  },
};
...
function presetToSlots(arr) {
  return arr.map(function (line) {
    return line.map(function (pos) {
      return { pos: pos, pid: null };
    });
  });
}
...
function showLoading() {
  ...
  div.id = "appLoader";
  div.style.cssText =
    "position:fixed;inset:0;z-index:9999;background:#050816;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;font-family:Cairo,sans-serif";
  div.innerHTML =
    '<div style="font-size:52px;animation:spin 1s linear infinite">⚽</div>' +
    "<div style=\"font-family:'Bebas Neue',cursive;font-size:28px;color:#38bdf8;letter-spacing:3px\">FUTURE ACADEMY</div>" +
    '<div style="font-size:13px;color:#475569">' +
    (USE_SUPABASE ? "جاري الاتصال بقاعدة البيانات..." : "جاري التحميل...") +
    "</div>" +
    ...
    try {
      var results = await Promise.all([
        ...
        sb(
          ...
          "GET",
          null,
          "?select=*&order=line_index,slot_index",
        ),
        sb("site_config", "GET", null, "?select=*"),
      ]);

      var playersRows = results[0],
        newsRows = results[1],
        trainingRows = results[2],
        weeklyRows = results[3],
        fmtCfgRows = results[4],
        fmtSlotRows = results[5],
        siteConfigRows = results[6];

      // Parse site_config key-value rows
      var siteConfig = {};
      if (siteConfigRows && siteConfigRows.length) {
        siteConfigRows.forEach(function (r) {
          siteConfig[r.key] = r.value;
        });
      }
      ...
      STATE = {
        ...
        training: (trainingRows || []).map(function (r) {
          return { id: r.id, day: r.day, focus: r.focus, time: r.time };
        }),
        weeklyTop: (weeklyRows || [])
          .filter(function (r) {
            return r.player_id;
          })
          .map(function (r) {
        ...
        formationName:
          (fmtCfgRows && fmtCfgRows[0] ? fmtCfgRows[0].name : null) || "4-3-3",
        formationAnimated:
          fmtCfgRows && fmtCfgRows[0] ? fmtCfgRows[0].animated : false,
        formationDay: ls.formationDay || "",
        savedFormations: ls.savedFormations || [],
        siteStatsOverride: {
          hero_tag:
            siteConfig.hero_tag ||
            (ls.siteStatsOverride && ls.siteStatsOverride.hero_tag) ||
            "",
          hero_text:
            siteConfig.hero_text ||
            (ls.siteStatsOverride && ls.siteStatsOverride.hero_text) ||
            "",
        },
      };

      if (!STATE.formationSlots || !STATE.formationSlots.length)
...
function _loadFromLS() {
  ...
  if (
    ...
    !Array.isArray(STATE.formationSlots) ||
    !STATE.formationSlots.length
  )
    STATE.formationSlots = defaultFormationSlots();
}
...
async function addPlayer(data) {
  if (USE_SUPABASE) {
    ...
    STATE.players.push(
      result && result[0]
        ? rowToPlayer(result[0])
        : Object.assign({}, data, { id: nextId(STATE.players) }),
    );
  } else {
...
async function updatePlayer(data) {
  ...
  saveState();
  if (USE_SUPABASE)
    await sb("players", "PATCH", playerToRow(data), "?id=eq." + data.id);
}
...
async function deletePlayer(id) {
  ...
  STATE.weeklyTop = STATE.weeklyTop.filter(function (x) {
    return x !== id;
  });
  STATE.formationSlots.forEach(function (line) {
    line.forEach(function (slot) {
      if (slot.pid === id) slot.pid = null;
    });
  });
  saveState();
...
async function addNews(data) {
  ...
  if (USE_SUPABASE) {
    ...
    if (result && result[0]) {
      ...
      STATE.news.unshift(
        Object.assign({}, data, { id: nextId(STATE.news), date: today }),
      );
  } else {
    STATE.news.unshift(
      Object.assign({}, data, { id: nextId(STATE.news), date: today }),
...
async function updateNews(data) {
  var i = STATE.news.findIndex(function (n) {
    return n.id === data.id;
  });
...
async function deleteNews(id) {
  STATE.news = STATE.news.filter(function (n) {
    return n.id !== id;
  });
  saveState();
...
async function addTraining(data) {
  if (USE_SUPABASE) {
    var result = await sb("training", "POST", {
      day: data.day,
      focus: data.focus,
      time: data.time,
    });
    if (result && result[0])
      STATE.training.push({
        id: result[0].id,
        day: result[0].day,
        focus: result[0].focus,
        time: result[0].time,
      });
    else
      STATE.training.push(
        Object.assign({}, data, { id: nextId(STATE.training) }),
      );
  } else {
    STATE.training.push(
      Object.assign({}, data, { id: nextId(STATE.training) }),
...
async function updateTraining(data) {
  var i = STATE.training.findIndex(function (t) {
    return t.id === data.id;
  });
  ...
  if (USE_SUPABASE)
    await sb(
      "training",
      "PATCH",
      { day: data.day, focus: data.focus, time: data.time },
      "?id=eq." + data.id,
    );
}

async function deleteTraining(id) {
  STATE.training = STATE.training.filter(function (t) {
    return t.id !== id;
  });
  saveState();
...
async function setWeeklyTop(arr) {
  ...
  if (USE_SUPABASE) {
    var rows = [1, 2, 3].map(function (rank) {
      return { rank: rank, player_id: arr[rank - 1] || null };
    });
    await sb("weekly_top", "POST", rows, "", {
...
async function _syncSlots() {
  ...
  if (rows.length)
    await sb("formation_slots", "POST", rows, "", { Prefer: "return=minimal" });
}
async function _syncConfig() {
  ...
  await sb(
    "formation_config",
    "PATCH",
    { name: STATE.formationName, animated: STATE.formationAnimated },
    "?id=eq.1",
  );
}

function setFormationSlots(slots) {
  STATE.formationSlots = slots;
  saveState();
  _syncSlots();
}
function setFormationName(n) {
  STATE.formationName = n;
  saveState();
  _syncConfig();
}
function setFormationDay(d) {
  STATE.formationDay = d;
  saveState();
}
function toggleAnimation() {
  STATE.formationAnimated = !STATE.formationAnimated;
  saveState();
  _syncConfig();
}
...
function addFormationLine() {
  STATE.formationSlots.push([{ pos: "CM", pid: null }]);
  saveState();
  _syncSlots();
}
function removeFormationLine(li) {
  if (STATE.formationSlots.length > 1) STATE.formationSlots.splice(li, 1);
  saveState();
  _syncSlots();
}
function addSlotToLine(li) {
  if (STATE.formationSlots[li])
    STATE.formationSlots[li].push({ pos: "CM", pid: null });
  saveState();
  _syncSlots();
}
function removeSlotFromLine(li, si) {
  if (STATE.formationSlots[li] && STATE.formationSlots[li].length > 1)
    STATE.formationSlots[li].splice(si, 1);
  saveState();
  _syncSlots();
}
function loadPresetFormation(key) {
  var preset = FORMATION_PRESETS[key];
  if (!preset) return;
...
function addFormationPlan(data) {
  ...
  STATE.savedFormations.push(
    Object.assign({}, data, { id: nextId(STATE.savedFormations) }),
  );
  saveState();
}
function deleteFormationPlan(id) {
  STATE.savedFormations = (STATE.savedFormations || []).filter(function (f) {
    return f.id !== id;
  });
  saveState();
}
...
function setSiteStatsOverride(ov) {
  STATE.siteStatsOverride = ov;
  saveState();
}
